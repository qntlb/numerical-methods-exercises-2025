 ...
 * order to generate the drawings of (X_1,X_2) and perform the computations.
 */
public class BivariateNormalTesting {

	// formatting to print a number with 5 decimal digits
	...
	/**
	 * Generates an object of the class to test the implementation of the two
	 * generation methods
	 *
	...
	/**
	 * Tests the precision and the efficiency of a selected method to generate a
	 * pair of independent normal random variables with expectation mu and standard
	 * deviation sigma. The method (inversion sampling, acceptance rejection) is selected through
	 * a switch statement based on an enum type containing the names of the methods.
	...
	public void lengthyTestMethod(NormalRandomVariable normalTestSampler, GenerationMethods method) throws Exception {
		...
		// Here we have a switch statement based on the name of the two methods, see the enum type GenerationMethods
		...
		case INVERSIONSAMPLING:
			...
			for (int i = 0; i < numberOfComputations; i++) {
				...
				long startTime = System.currentTimeMillis();// time when the computations starts
				...
				long endTime = System.currentTimeMillis();// time when the computation ends: it depends on the method.
		...
		case ACCEPTANCEREJECTION:
			...
			for (int i = 0; i < numberOfComputations; i++) {
				...
				for (int j = 0; j < numberOfDrawingsPerComputation; j++) {
					double[] generatedPair = normalTestSampler.generateBivariateAR();
					if (generatedPair[0] < mu && generatedPair[1] < mu) {
						numberOfTimesBothSmallerThanMu++;
					}
				}
	...
	public void testMethod(NormalRandomVariable normalTestSampler,GenerationMethods method) throws Exception {
		...
		for (int i = 0; i < numberOfComputations; i++) {
			...
			final long startTime = System.currentTimeMillis();// time when the computations starts
			for (int j = 0; j < numberOfDrawingsPerComputation; j++) {
				/*
