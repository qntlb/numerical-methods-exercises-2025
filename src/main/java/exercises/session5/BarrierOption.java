...
/**
 * This class implements the valuation of a European barrier option on a single asset.
 * In particular, the option has payoff
 * max(X_T-K, 0)1_{B_L <= X_t <= B_U} for a given underlying X_t, t >=0. 
 * Here K is the strike of the option, T its maturity, B_L and B_U the lower and upper barrier, respectively.
 * 
 * This class extends AbstractAssetMonteCarloProduct, implementing the method
 * getValue(double evaluationTime, AssetModelMonteCarloSimulationModel model).
 * 
 * We do that using the methods of the interface AssetModelMonteCarloSimulationModel.
 */
public class BarrierOption extends AbstractAssetMonteCarloProduct {
	...
	/**
	 * Constructs an object representing a European barrier option on an underlying X.
	 * @param maturity The maturity T in the option payoff max(X_T-K,0)1_{B_L <= X_t <= B_U}
	 * @param strike The strike K in the option payoff max(X_T-K,0)1_{B_L <= X_t <= B_U}
