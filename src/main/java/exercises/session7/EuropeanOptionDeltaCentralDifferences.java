...
/**
 * This class implements the approximation of the delta of a European call option through finite central differences,
 * using the Monte-Carlo method in order to get the prices of the European call options for shifted initial values.
 * In particular, the delta of the option is approximated as
 * (V(S_0+delta)-V(S_0-delta))/(2 delta),
 * where S_0 is the initial value of the underlying and V(x) is the Monte-Carlo price of the option when the
...
public class EuropeanOptionDeltaCentralDifferences extends AbstractAssetMonteCarloProduct {
	...
	/**
	 * It constructs an object that computes the approximated value of the delta of the option.
	 *
	...
	/**
	 * This method returns the value of the delta of a European call option through central differences, as a
	 * RandomVariable.
	 *
	 * @param evaluationTime   The time on which the delta of the option value is evaluated.
	 * @param underlyingSimulation, object of type AssetModelMonteCarloSimulationModel.
	...
	public RandomVariable getValue(double evaluationTime, AssetModelMonteCarloSimulationModel underlyingSimulation)
		...
		// First thing, we construct a EuropeanOption object with maturity and strike that are fields of the class
