...
/**
 * This class has two methods that compute the central finite difference approximation of the delta of a call
 * option for the Black-Scholes model: one method does it by computing the analytical prices for the shifted
...
public class FiniteDifferencesBlackScholes {
	...
	/**
	 * It constructs an object that computes the approximated delta of a European call option for a Black-Scholes
	 * underlying.
	 *
	 * @param riskFreeRate
	 * @param volatility
	 ...
	 *
	 * @param times,               the TimeDiscretization object representing the times at which we discretize
	...
	public double centralFiniteDifferenceApproximationBlackScholes(TimeDiscretization times, int numberOfSimulations,
		...
		/*
		 * Note that, with respect to the case where we just computed the analytic values of the option, we now
	 ...
	 * both by computing the analytical prices and approximating the prices by Monte-Carlo. The plot is done with respect to the
	...
	public void plotForwardFiniteDifferenceApproximationErrorOfBlackScholes(TimeDiscretization times,
		...
		Plot2D plot = new Plot2D(-15, -1, 28,
